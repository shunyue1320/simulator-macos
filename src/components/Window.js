...
function TrafficLights({ setShow, max, setMax }) {
  ...
  return (
    <div className="flex flex-row absolute left-0 space-x-2 pl-2 mt-1.5">
      <button onClick={closeWindow} className="w-3 h-3 rounded-full bg-red-500 outline-none focus:outline-none" />
      <button onClick={() => setMax(false)} className="w-3 h-3 rounded-full bg-yellow-500 outline-none focus:outline-none" />
      <button onClick={() => setMax(!max)} className="w-3 h-3 rounded-full bg-green-500 outline-none focus:outline-none" />
    </div>
  )
}

export default function Window({ content, title, setShow, max, setMax, active, z, size }) {
  ...
  return (
    ...
    >
      <div onDoubleClick={() => setMax(!max)} className="relative h-6 text-center bg-gray-300">
